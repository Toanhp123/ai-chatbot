...
def configure_optimizer(model: nn.Module, config: TrainingConfig) -> torch.optim.Optimizer:
    """
    ...
    if hasattr(model, "configure_optimizers") and callable(getattr(model, "configure_optimizers")):
        custom_optim = getattr(model, "configure_optimizers")(config)
    ...
    opt_type = config.optimizer_type.lower().strip()
    if opt_type == "sgd":
        return torch.optim.SGD(optim_groups, lr=config.learning_rate, momentum=config.sgd_momentum)
    ...
    if opt_type == "8bit_adamw":
        try:
            ...
            logger.warning(
                "Thư viện 'bitsandbytes' không được cài đặt. Tự động fallback về AdamW 32-bit."
            )
