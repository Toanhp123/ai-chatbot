...
The repository has no project-local development Skills. **Use Superpowers** for non-trivial engineering, **use `ui-ux-pro-max`** for substantial UI work, and **use Graphify** for broad structural repository analysis once substantive code exists. `frontend-design` and Ponytail remain optional refinement capabilities. `DEVELOPMENT_TOOLING.md` owns routing, blocking, privacy, freshness, and evidence policy. Product Skills/extensions in `MCP_AND_EXTENSIONS.md` are application features and must not be confused with development tooling.
...
| `AGENT_RUNTIME.md` | normative | Agent Task state machine, orchestration, cancellation, subagents, events/recovery |
| `TOOL_RUNTIME.md` | normative | tool contract, permission mediation, execution, audit, sandbox seam |
| `PROVIDER_SYSTEM.md` | normative | provider/model registry, normalization, capabilities, usage/errors/routing |
...
| `MODEL_LAB.md` | normative | dataset/training worker, SFT/LoRA/QLoRA, evaluation/export |
...
| `EXECUTION_PROTOCOL.md` | operational | autonomous bootstrap loop, stop conditions, deterministic E2E gate |
| `IMPLEMENTATION_STATUS.md` | ledger | durable factual progress/resume/evidence |
| `ROADMAP.md` | charter/operational | vertical milestones and acceptance criteria |
...
- **MCP/extensions:** `MCP_AND_EXTENSIONS.md`, `TOOL_RUNTIME.md`, `SECURITY.md`, `CONTRACT_INVENTORY.md`;
- **persistence/migration:** `DATA_MODEL.md`, `ARCHITECTURE.md`, `OPERATIONS_AND_PRIVACY.md`, `TEST_STRATEGY.md`;
- **local inference:** `LOCAL_AI.md`, `PROVIDER_SYSTEM.md`, `OPERATIONS_AND_PRIVACY.md`;
- **Model Lab:** `MODEL_LAB.md`, `LOCAL_AI.md`, `SECURITY.md`, `OPERATIONS_AND_PRIVACY.md`;
...
- `../AGENTS.md` owns always-on agent behavior and capability gates; `DEVELOPMENT_TOOLING.md` owns task routing, blocking semantics, and safe operating policy for those capabilities.
